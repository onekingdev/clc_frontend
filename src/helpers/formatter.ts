  ...
  keys.forEach((key: string, index) => {
    switch (key) {
      case "sunday":
        list[0] = {
          name: key,
          "Questions Correct": values[index].correct,
          "Tickets Earned": values[index].tickets,
        };
        break;
      case "monday":
        list[1] = {
    ...
    responseParser.forEach((item: any) => {
      if (
        ...
        response = response.replace(
          item.word,
          spanWord(item.word, item.definition)
        );
      }
    });
  }
...
export const UTGLabeling = (dealer: number, players: any) => {
  ...
  players.forEach((player: any, index: number) => {
    if (index === dealerIndex) playerLabels[index] = labels[0];
      ...
      index === dealerIndex + 1 ||
      index === dealerIndex + 1 - totalPlayers
    )
      playerLabels[index] = labels[1];
    else if (
      index === dealerIndex + 2 ||
      index === dealerIndex + 2 - totalPlayers
    )
      playerLabels[index] = labels[2];
    else if (
      index === dealerIndex + 3 ||
      index === dealerIndex + 3 - totalPlayers
    )
      playerLabels[index] = labels[3];
    else if (
      index === dealerIndex + 4 ||
      index === dealerIndex + 4 - totalPlayers
    )
      playerLabels[index] = labels[4];
    else if (
      index === dealerIndex + 5 ||
      index === dealerIndex + 5 - totalPlayers
    )
      playerLabels[index] = labels[5];
    else if (
      index === dealerIndex + 6 ||
      index === dealerIndex + 6 - totalPlayers
    )
      playerLabels[index] = labels[6];
    else if (
      index === dealerIndex + 7 ||
      index === dealerIndex + 7 - totalPlayers
    )
      playerLabels[index] = labels[7];
    else if (
      index === dealerIndex + 8 ||
      (dealerIndex + 8 > totalPlayers &&
        index === dealerIndex + 8 - totalPlayers)
    )
      playerLabels[index] = labels[8];
  });
